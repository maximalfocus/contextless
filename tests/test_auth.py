...
_PAYLOAD = {"body": "Order {{ order.number }}", "order_reference": "1001"}


def test_missing_credentials_are_generic_401(client: TestClient) -> None:
    response = client.post("/notifications/preview", json=_PAYLOAD)
...
def test_unknown_token_is_generic_401(client: TestClient) -> None:
    response = client.post("/notifications/preview", headers=auth("nope-not-real"), json=_PAYLOAD)
    assert response.status_code == 401
    assert response.headers.get("WWW-Authenticate") == "Bearer"


def test_all_rejections_are_indistinguishable(client: TestClient) -> None:
    missing = client.post("/notifications/preview", json=_PAYLOAD)
    malformed = client.post(
        "/notifications/preview",
        headers={"Authorization": "Basic Zm9vOmJhcg=="},
        json=_PAYLOAD,
    )
    unknown = client.post("/notifications/preview", headers=auth("nope-not-real"), json=_PAYLOAD)
...
def test_valid_token_is_accepted(client: TestClient) -> None:
    response = client.post("/notifications/preview", headers=auth(GLOBEX_TOKEN), json=_PAYLOAD)
